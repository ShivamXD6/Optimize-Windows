...
reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" /v "Shell" /t REG_SZ /d "explorer.exe, cmd /c \"%~dpnx0\"" /f
shutdown /r /f /t 1
)
"@
...
function Update-ShivaayFolderInRegistry {
    ...
    Set-ItemProperty -Path $customPath -Name "ShivaayFolderPath" -Value $newShivaayFolder
